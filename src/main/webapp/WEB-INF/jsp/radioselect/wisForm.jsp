...
<script>
	...
	// 1. Controller -> select01/02 VO 값을 var select1/2 에 저장한다.
	// selectRadioSelectVO 
	// JSTL 값을 받아오기위해선 c:out value 를 사용 해야한다.
	// 서치 했을때는 없이 사용해도 된다고 하지만, 사용이안되서, c:out 을 사용하라해서 사용해보니 돌아감 .
	// 내가 select box 에서 선택했었던 값을 var select1 에 저장한다.	
	
	var selectbox01 = '<c:out value="${selectRadioSelectVO.select1}"/>';
	var selectbox02 = '<c:out value="${selectRadioSelectVO.select2}"/>';
	
	// 2. 1번 2번 셀렉트 박스에서 폼으로 들어갔을때.
	// 샐랙트 박스를 들어갔을때 내가 선택했던 값을 select1,2 에 벨류값으로 해서
	// 체크가 되게금 하는것이다. 
